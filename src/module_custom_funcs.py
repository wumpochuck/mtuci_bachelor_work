...
def move_to(x, y, duration=None):
    """Перемещает курсор со случайной скоростью."""
    if duration is None:
...
def type_text(text, interval=None):
    """Вводит текст, имитируя печать человека."""
    if interval is None:
...
def press_key(key, presses=1):
    ...
    try:
        ...
        pg.press(key, presses=presses, interval=random.uniform(0.1, 0.2))
        human_delay()
    except Exception as e:
        logger.log("error", f"Ошибка при нажатии {key}", str(e))

def hotkey(*args):
    """Универсальная функция для горячих клавиш."""
    try:
...
def win_r_run(command):
    """Специфическое действие: запуск через Win+R"""
    hotkey('win', 'r')
    time.sleep(0.5)
    type_text(command)
    press_key('enter')
